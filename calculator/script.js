...
window.update = function() {
  ...
  const mapping = {
    ...
    's2-inv': '~' + fmtE(panels * POWER_PER_PANEL * (parseFloat(document.getElementById('sl-prix-wc')?.value || 1.1))),
    's3-inv': '~' + fmtE(panels * POWER_PER_PANEL * (parseFloat(document.getElementById('sl-prix-wc')?.value || 1.1)) + (parseFloat(document.getElementById('sl-prix-batt')?.value || 7000))),
    ...
    's4-maint': 'Inclus ✓',
    's1-ass': '0 €',
    's2-ass': fmtE(dS2.reduce((acc, d) => acc + d.insurance, 0)),
    's3-ass': fmtE(dS3.reduce((acc, d) => acc + d.insurance, 0)),
    's4-ass': 'Inclus ✓',
    'g-s1': fmtE(total25S1),
    'g-s4': fmtE(total25S4)
  };
